...
function OnIncinerateHit(keys)
	...
	if caster:HasModifier("modifier_aspd_increase") then
		DoDamage(caster, target, keys.ExtraDamage*currentStack*3, DAMAGE_TYPE_PHYSICAL, 0, keys.ability, false)
	else
		DoDamage(caster, target, keys.ExtraDamage*currentStack, DAMAGE_TYPE_PHYSICAL, 0, keys.ability, false)
	end
end
