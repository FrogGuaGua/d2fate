...
BLESSING_PERIOD = 480
BLESSING_MANA_REWARD = 10   --Round 10
...
model_lookup["npc_dota_hero_storm_spirit"] = "models/jack/jack.vmdl"
model_lookup["npc_dota_hero_axe"] = "models/lvbu/lvbu.vmdl"
...
function Precache( context )
    ...
    PrecacheResource( "soundfile", "soundevents/hero_jack.vsndevts", context )
...
function FateGameMode:OnNPCSpawned(keys)
    ...
    if hero.restart_music then
        hero.restart_music = nil
        CustomGameEventManager:Send_ServerToPlayer(hero:GetPlayerOwner(), "fate_restart_music", {})
    end
...
function FateGameMode:OnEntityKilled( keys )
    ...
    if killedUnit:IsRealHero() then
        ...
        if killedUnit:GetName() == "npc_dota_hero_mirana" and killedUnit.oncombo then
...
function FateGameMode:InitGameMode()
    ...
    -- Yes can be done but I don't wish to reverse this just yet because i don't wanna screw something up
...
function FateGameMode:TakeDamageFilter(filterTable)
    ...
    -- Tamamo Fox's Wedding
    if attacker:HasModifier("modifier_foxs_wedding") ~= victim:HasModifier("modifier_foxs_wedding") and damageType ~= DAMAGE_TYPE_PURE then
        return false
    end
