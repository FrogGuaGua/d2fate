...
function OnCosmicOrbitAttackLanded(keys)
	...
	if caster:HasModifier("modifier_lishuwen_cosmic_orbit") then
...
function OnTigerStrike2Start(keys)
	...
	keys.target:AddNewModifier(caster, target, "modifier_stunned", {Duration = 0.4})
    ...
    target:OnPhysicsFrame(function(unit) 
		...
		if diff:Length() > (500 + caster:GetStrength() * 4 ) then
			...
			local push_particle = ParticleManager:CreateParticle("particles/units/heroes/hero_monkey_king/taunt_dust_impact.vpcf",  PATTACH_ABSORIGIN_FOLLOW, target)
	...
	target:OnPreBounce(function(unit, normal) -- stop the pushback when unit hits wall
		...
		local block_particle = ParticleManager:CreateParticle("particles/units/heroes/hero_tusk/tusk_snowball_impact_mist.vpcf",  PATTACH_ABSORIGIN_FOLLOW, target)
		ParticleManager:SetParticleControlEnt(block_particle, 1, target, PATTACH_POINT_FOLLOW, "attach_hitloc", keys.target:GetAbsOrigin(), false)
		target:EmitSound("Hero_EarthShaker.Fissure")
		DoDamage(caster, target, 400 + caster:GetStrength() * 3, DAMAGE_TYPE_PHYSICAL, 0, keys.ability, false)	
	end)
	Timers:CreateTimer(2.0, function()
		ParticleManager:DestroyParticle( block_particle, false )
		ParticleManager:DestroyParticle( push_particle, false  )
	end)
...
function OnTigerStrike3Start(keys)
	...
	local ability = keys.ability
	--target:TriggerSpellReflect(ability)
	--if TigerStrikeCheckTarget(caster, keys.tigerStrikeTarget, ability) then return end
	--if IsSpellBlocked(keys.target) then return end
    --target = keys.tigerStrikeTarget
	if caster:GetUnitName() == "npc_dota_hero_bloodseeker" then
		GrantCosmicOrbitResist(caster)
	end
	if caster.bIsMartialArtsImproved then
	...
	-- do damage and apply CC
	local damage = keys.BaseDamage + caster:GetAgility() * 5 +caster:GetStrength()
	if target:GetMaxHealth() * 0.4 > target:GetHealth() then
		damage = damage *2
	else
		local losthealth = target:GetMaxHealth()-target:GetHealth()
		damage= damage * (1 + losthealth/target:GetMaxHealth() * 1.66)
	end
	if caster.bIsFuriousChainAcquired then
...
function OnNSSDelayFinished(keys)
	...
	if target:HasModifier("modifier_mark_of_fatality") then
		local abil = caster:FindAbilityByName("lishuwen_martial_arts")
		local currentStack = target:GetModifierStackCount("modifier_mark_of_fatality", abil)
		damage = target:GetMaxHealth() * ATTR_NSS_STACK_DAMAGE_PERCENTAGE * currentStack/100
	end
