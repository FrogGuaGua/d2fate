...
JackAttribute = {
	"jack_attribute_information_erasure",
	"jack_attribute_female_slayer",
	"jack_attribute_ushiro",
	"jack_attribute_murder",
	"jack_the_mist",
	attrCount = 5
}
LvbuAttribute = {
...
function FindAttribute(name)
	...
	elseif name == "npc_dota_hero_storm_spirit" then
		attributes = JackAttribute
	elseif name == "npc_dota_hero_axe" then
		attributes = LvbuAttribute
	end
    return attributes
end 
...
function OnDamageGain(keys)
	...
	hero:SetBaseDamageMax(hero:GetBaseDamageMax() - math.floor(primaryStat) + 4)
	hero:SetBaseDamageMin(hero:GetBaseDamageMin() - math.floor(primaryStat) + 4)
	hero:CalculateStatBonus()
...
function OnArmorGain(keys)
	...
	--hero:SetPhysicalArmorBaseValue(hero:GetPhysicalArmorBaseValue()+1)
	hero:FindModifierByName("modifier_attr_armor"):SetStackCount(hero.ARMORgained)
	hero:CalculateStatBonus()
	-- Set master 1's mana 
	local master1 = hero.MasterUnit
	master1:SetMana(master1:GetMana() - keys.ability:GetManaCost(keys.ability:GetLevel()))
...
function OnHPRegenGain(keys)
	...
	hero:FindModifierByName("modifier_attr_hpregen"):SetStackCount(hero.HPREGgained)
	hero:CalculateStatBonus()
	-- Set master 1's mana 
	local master1 = hero.MasterUnit
	master1:SetMana(master1:GetMana() - keys.ability:GetManaCost(keys.ability:GetLevel()))
...
function OnManaRegenGain(keys)
	...
	hero:FindModifierByName("modifier_attr_manaregen"):SetStackCount(hero.MPREGgained)
	hero:CalculateStatBonus()
	-- Set master 1's mana 
	local master1 = hero.MasterUnit
	master1:SetMana(master1:GetMana() - keys.ability:GetManaCost(keys.ability:GetLevel()))
...
function OnHeroRespawn(keys)
	local caster = keys.caster
	local ability = keys.ability
	if caster:GetName()==("npc_dota_hero_mirana") and caster.oncombo then 
