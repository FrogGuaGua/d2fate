...
function OnPCFAcquired(keys)
	...
	if not caster.spellbook_open then hero:SwapAbilities("fate_empty1", "tamamo_polygamist_castration_fist", false, true) end
...
function TamamoCheckCombo(caster, ability)
	if caster:GetStrength() >= 19.1 and caster:GetAgility() >= 19.1 and caster:GetIntellect() >= 19.1 and not caster.IsEscapeAcquired then
		if ability == caster:FindAbilityByName("tamamo_mantra") and caster:FindAbilityByName("tamamo_polygamist_castration_fist"):IsCooldownReady() then
			if not caster.spellbook_open then caster:SwapAbilities("fate_empty1", "tamamo_polygamist_castration_fist", false, true) end
			caster.combo_ready = true
			Timers:CreateTimer(3, function()
				if not caster.spellbook_open and not caster.IsEscapeAcquired then caster:SwapAbilities("fate_empty1", "tamamo_polygamist_castration_fist", true, false) end
				caster.combo_ready = false
			end)
		end
	end
end
