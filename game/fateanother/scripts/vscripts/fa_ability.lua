...
function OnHeartDamageTaken(keys)
	...
	if damageTaken > keys.Threshold and caster:GetHealth() ~= 0 then
		...
		if (caster:GetAbsOrigin()-target:GetAbsOrigin()):Length2D() < 1500 and not target:IsInvulnerable() and caster:GetTeam() ~= target:GetTeam() then
...
function OnQuickdrawHit(keys)
	...
	local damage = 500 + keys.caster:GetAgility() * 15
...
function OnTGStart(keys)
	...
	if caster:GetName() == "npc_dota_hero_juggernaut" then
		...
		-- 1.27d caster:FindAbilityByName("false_assassin_gate_keeper"):StartCooldown(keys.GCD) 
    ...
    Timers:CreateTimer(0.2, function()
		caster:AddNewModifier(caster, nil, "modifier_phased", {duration=0.8})
	giveUnitDataDrivenModifier(caster, caster, "dragged", 0.8)
	giveUnitDataDrivenModifier(caster, caster, "revoked", 0.8)
	end)
	--[[if caster.IsGanryuAcquired then 
...
function OnEyeOfSerenityAcquired(keys)
    ...
    hero.IsEyeOfSerenityActive = true
	-- Set master 1's mana 
	local master = hero.MasterUnit
	master:SetMana(master:GetMana() - keys.ability:GetManaCost(keys.ability:GetLevel()))
end
