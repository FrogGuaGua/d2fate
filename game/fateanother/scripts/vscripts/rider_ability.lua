...
function StartMysticEyesTimer(keys)
	...
	Timers:CreateTimer('MysticEyes_passive_timer', {
		...
		for k,v in pairs(targets) do
			if v ~= caster then 
				if IsFacingUnit(caster, v, 120) then
...
function OnMonstrousStrengthProc(keys)
	DoDamage(keys.caster, keys.target,60 + keys.caster:GetStrength(), DAMAGE_TYPE_MAGICAL, 0, keys.ability, false)
	if keys.target:HasModifier("modifier_breaker_gorgon") or keys.target:HasModifier("modifier_bloodfort_slow") then
		giveUnitDataDrivenModifier(keys.caster,keys.target, "stunned", 0.1)
		DoDamage(keys.caster, keys.target, 25+keys.caster:GetIntellect(), DAMAGE_TYPE_PURE, 0, keys.ability, false)
	end
end
