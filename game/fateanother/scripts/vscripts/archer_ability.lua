...
function OnUBWLevelUp(keys)
	...
	caster:FindAbilityByName("archer_5th_gae_bolg"):SetLevel(keys.ability:GetLevel())
end
...
function EndUBW(caster)
	...
	--for k=1,#ubwTargets do
		--if (not ubwTargets[k]:IsAlive()) and ubwTargets[k]:GetName() == "npc_dota_hero_mirana" then  --(not ubwTargets[k]:IsNull()) and 
			--table.insert(units,ubwTargets[k])    --1.18  fixed ruler combo by rind
		--end
	--end
	...
	Timers:CreateTimer( function()
		if caster:HasModifier("modifier_sword_barrage") then			
			...
			Timers:CreateTimer( delay, function()
					...
					for k,v in pairs(targets) do
						DoDamage(caster, v, keys.Damage+caster:GetIntellect() , DAMAGE_TYPE_MAGICAL, 0, keys.ability, false)
...
function OnArcher5thGBstart(keys)
	...
	if caster.IsProjectionImproved then
		damage = damage + caster:GetIntellect() * 3
	end
	...
	if caster.IsProjectionImproved and caster:HasModifier("modifier_ubw_death_checker") then
		stunDuration = stunDuration + 0.55
		damage = damage + 200
	end
	...
	local dagon_particle = ParticleManager:CreateParticle("particles/econ/events/ti4/dagon_ti4.vpcf",  PATTACH_ABSORIGIN_FOLLOW, keys.caster)
...
function OnOveredgeAcquired(keys)
	...
	hero:SwapAbilities("archer_5th_gae_bolg", "archer_5th_overedge", false, true) 
