...
donotlevel = {
    ...
    "archer_5th_overedge",
    "jack_information_erasure_passive"
}
        ...
        if currentItem then
            ...
            if GetMatchingItem(currentItemName1) then
                ...
                for j=0,5 do
                    ...
                    if j == currentItemIndex1 then 
                        goto continue 
                    end -- just continue if we are looking at the same slot as first component
...
function IsRevoked(target)
    ...
    local tamamo_revoke = target:FindModifierByName("modifier_chaos_heaven_revoke")
    if tamamo_revoke and tamamo_revoke:IsRevoked() then return true end
...
function GetPhysicalDamageReduction(armor)
    local reduction = (0.052 * armor)/(0.9+ 0.048 * armor)
    if armor < 0 then reduction = armor * 0.03 end
    return reduction
    --[[if armor >= 0 then 
        return reduction
    else
   ...
   -- if target == nil then return end 
    ...
    if dmg_type == DAMAGE_TYPE_MAGICAL then
        ...
        if IsBScrollIgnored == false and target:HasModifier("modifier_b_scroll") then 
            ...
            target.BShieldAmount = target.BShieldAmount - dmg
...
local heroNames = {
    ...
    ["npc_dota_hero_storm_spirit"] = "Jack",
    ["npc_dota_hero_axe"]="Berserker(Extra), LvBu"
}
...
local heroCombos = {
    ...
    ["npc_dota_hero_storm_spirit"] = "jack_the_mist",
}
