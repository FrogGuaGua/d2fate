...
function OnVengeanceStart(keys)
	...
	if caster.IsDIAcquired then
		DoDamage(caster, target, keys.Damage + 100, DAMAGE_TYPE_MAGICAL, 0, keys.ability, false)
	else
		DoDamage(caster, target, keys.Damage, DAMAGE_TYPE_MAGICAL, 0, keys.ability, false)
	end
...
function OnVengeanceEnd(keys)
	...
	local target = keys.target
	if caster.IsDIAcquired then
		DoDamage(target, caster, keys.Damage * keys.ReturnAmount/100 + caster:GetStrength() * 3, DAMAGE_TYPE_PURE, 0, keys.ability, false)
	else
...
function OnBloodStart(keys)
	...
	--ability:ApplyDataDrivenModifier(caster, target, "modifier_blood_curse_debuff", {duration=10.0})
...
function OnBloodCurseStart(keys)
	--if not caster.IsBloodMarkAcquired then return end
	local caster = keys.caster
	if caster.IsBloodMarkAcquired then
	    local target = keys.attacker
	    caster:EmitSound("Avenger.Consume")
	    DoDamage(caster, target, 444, DAMAGE_TYPE_PURE, 0, keys.ability, false)	
	    keys.ability:ApplyDataDrivenModifier(caster, target, "modifier_blood_curse_debuff", {duration=15.0})
	end
end

function OnBCTick(keys)
	local caster=keys.caster
	local target=keys.target
	--target:EmitSound("avenger_attack_01")
	DoDamage(caster, target, 50, DAMAGE_TYPE_PURE, 0, keys.ability, false)
	SpawnAttachedVisionDummy(caster, target, 200, 1, true)
end
function OnOverdriveAcquired(keys)
    local caster = keys.caster
    local ply = caster:GetPlayerOwner()
	local hero = caster:GetPlayerOwner():GetAssignedHero()
	hero.IsOverdriveAcquired = true
