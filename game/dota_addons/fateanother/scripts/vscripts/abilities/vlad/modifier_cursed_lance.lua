...
function modifier_cursed_lance:GetAttributes()
  return MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE
end

function modifier_cursed_lance:DeclareFunctions()
	local funcs = {
...
function modifier_cursed_lance:VFX0_Counter(parent)
...
function modifier_cursed_lance:VFX1_Shield(parent)
...
function modifier_cursed_lance:VFX2_PreExplosion(parent)
...
function modifier_cursed_lance:VFX3_Explosion(parent)
...
function modifier_cursed_lance:VFX4_AOEIndicator(parent)
...
function modifier_cursed_lance:VFX5_ExpiringIndicator()
...
function modifier_cursed_lance:OnDestroy()
...
function modifier_cursed_lance:UpdateModVars()
	local ability = self:GetAbility()
	self.CL_MAX_DMG = ability:GetSpecialValueFor("max_dmg")
	self.CL_MAX_SHIELD = ability:GetSpecialValueFor("max_shield")
	self.__cl_prev_amount = 7052 --its needed to put smh in this var after all
end

function modifier_cursed_lance:OnCreated()
	local parent = self:GetParent()
	self:UpdateModVars()
...
function modifier_cursed_lance:OnRefresh()
	self:OnDestroy()
	self:OnCreated()
end

function modifier_cursed_lance:ApplyAttrBonuses(parent)
	local parent = self:GetParent()
...
--shieldstuff
function modifier_cursed_lance:OnTakeDamage(keys)
	local parent = self:GetParent()
	local ability = self:GetAbility()
	if keys.unit == parent and self.CL_SHIELDLEFT ~= 0 then
...
--explosion
function modifier_cursed_lance:OnRemoved()
	local parent = self:GetParent()
	local ability = self:GetAbility()
...
function modifier_cursed_lance:OnIntervalThink()
...
function modifier_cursed_lance:IsHidden()
  return false
end

function modifier_cursed_lance:IsDebuff()
  return false
end

function modifier_cursed_lance:RemoveOnDeath()
  return true
end
