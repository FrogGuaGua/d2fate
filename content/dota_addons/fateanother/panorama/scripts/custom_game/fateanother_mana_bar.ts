...
class ManaBar {
    dota_mana: Panel;
    custom_mana: Panel;
    custom_mana_label: LabelPanel;
    custom_mana_progress: ProgressBar;

    constructor(){
        const root = GetHUDRootUI();
        this.dota_mana = root.FindChildTraverse("ManaContainer");
        this.BuildCustomMana();
        GameEvents.Subscribe("dota_player_update_selected_unit", () => {this.EventUpdateSelection()});
    ...
    Update(){
        ...
        if(Entities.GetUnitName(unit) === "npc_dota_hero_phantom_assassin") {
            const mana_info = CustomNetTables.GetTableValue("sync", unit + "_mana");
    ...
    SetShowDefaultMana(show: boolean){
        this.dota_mana.visible = show;
        this.custom_mana.visible = !show;
    }

    BuildCustomMana(){
        const root = GetHUDRootUI().FindChildTraverse("HealthManaContainer");
        ...
        const progress_left = this.custom_mana_progress.FindChildTraverse("CustomManaProgress_Left");
        ...
        const dota_scene_container = $.CreatePanel("Panel", progress_left, "");
        ...
        const progress_right = this.custom_mana_progress.FindChildTraverse("CustomManaProgress_Right");
